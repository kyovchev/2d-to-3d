...
gt_cloud = np.fromfile('0000000854.bin', dtype=np.float32).reshape(-1, 4)[:, :3]  # XYZ само

#  Зареждане на реконструирани точки от DUST3R.
pcd = o3d.io.read_point_cloud('pointcloud_854.ply')
...
source_pcd.rotate(source_pcd.get_rotation_matrix_from_xyz((np.pi / 2, np.pi / 2 + 0.1, np.pi)), center=(0, 0, 0))
source_pcd.scale(26, center=(0, 0, 0))
